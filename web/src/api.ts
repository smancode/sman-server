...
export const api = {
  ...
  getHubDevMode: (token: string) => request('/hub-dev-mode', token),
  setHubDevMode: (token: string, enabled: boolean) =>
    request('/hub-dev-mode', token, { method: 'PUT', body: JSON.stringify({ enabled }) }),
